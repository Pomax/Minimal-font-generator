...
TinyFontGenerator = {
   ...
   */
  getOS2Data: function () {
    if (this.OS2data !== false) {
      return this.OS2data;
    }

    var data = "";
    data += " 00 04"; // The current (Jan 2012) OS/2 table version is 5
  ...
  getCMAPData: function (charnum, hexchar) {
    if (this.CMAPdata !== false && this.codedCharacter === hexchar) {
  ...
  getHEADData: function () {
    ...
    data += " 00 00"; // (fontDirectionHint is deprecated and should be 2, but we use 0 anyway)
  ...
  getHHEAData: function () {
    if (this.HHEAdata !== false) {
      return this.HHEAdata;
    }

    var data = "";
    data += " 00 01 00 00"; // table version
    data += " 00 00"; // typographical ascender
    data += " 00 00"; // typographical descender
    data += " 00 00"; // typographical line gap
  ...
  /**
   * Get the NAME table data, or set it up if it doesn't exist yet.
   */
  getNAMEData: function () {
    if (this.NAMEdata !== false) {
  ...
  generateFont: function (character) {
    ...
    for (var i = 0; i < olen; i++) {
      ...
      while (tables[tablename].length % 4 != 0) {
        tables[tablename] += this.chr(0);
      }
      font += tablename;
